...
elasticsearch.prototype.get = function(limit, offset, callback){
    ...
    }else if(type === 'mappings,settings'){
        self.getMappingsAndSettings(limit, offset, callback);
    }else{
        callback(new Error('unknown type option'), null);
    }
};

elasticsearch.prototype.getMappingsAndSettings = function (limit, offset, callback){
    var self = this;
    if(self.gotMappingsAndSettings === true){
        callback(null, []);
    }else{
        var url = self.baseUrl + '/_settings,_mapping';
        request.get(url, function(err, response){
            self.gotMappingsAndSettings = true;
            var payload = [];
...
elasticsearch.prototype.set = function(data, limit, offset, callback){
    ...
    if(type === 'data'){
        ...
        self.setMappingsAndSettings(data, limit, offset, callback);
    }else{
        callback(new Error('unknown type option'), null);
    }
};

elasticsearch.prototype.setMappingsAndSettings = function(data, limit, offset, callback){
    var self = this;
    if(self.haveSetMappingsAndSettings === true){
        callback(null, 0);
    }else{
        ...
        for(var index in data){
            var mappings_and_settings = data[index];
            started++;
            count++;
            var payload = {
                    url: self.baseUrl,
                    body: JSON.stringify(mappings_and_settings),
                    timeout: self.parent.options.timeout,
            };
            request.put(payload, function(err, response){ // ensure the index exists
                started--;
                if(err){
                ...
                if(started === 0){
                    self.haveSetMappingsAndSettings = true;
                    callback(err, count);
                }
            });
        }
